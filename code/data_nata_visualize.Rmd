...
```{r}
# try to just make a single map
  plot_df <- map_df %>%
    select("BENZENE", long, lat, order, hole, piece, Tract, group)
  colnames(plot_df)[1] <- "value"
  plot_df %>%
    ggplot(aes(x=long, y=lat, group=group)) + 
    geom_polygon(aes(fill=value), show.legend = T) +
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          axis.text = element_blank(),
          axis.ticks = element_blank())
  ggsave(filename = "/home/guest/spatial-ses-lcm/alex/v2/Plots/NATA_data/NATA_2014_NC_BENZENE.png")
